...
input:
*  $i(t)$ [mg/hr]: rate at which the drug is ingested

outputs:
*  $g(t)$ [mg/L]: drug concentration in GI tract
*  $b(t)$ [mg/:]: drug concentration in blood

(constant) parameters:
*  $V_g$ [L]: volume of fluid in the GI tract
*  $V_b$ [L]: volume of blood in patient
*  $\beta$ [hr⁻¹]: rate constant for absorption of drug into blood from GI tract
*  $\alpha$ [hr⁻¹]: rate constant for decay of the drug in the blood
...
md"🍕 visualize the poles of the second order \"overall\" transfer function $B(s)/I(s)$ in the complex plane. note if the poles are real or have imaginary parts and if they lie in the left- or right-half plane.
...
in `Controlz.jl`, simulate the response $b(t)$ to this pill-swallowing input if $A=10$ mg. 
...
md"🍕 when (at what time $t$) does the peak drug concentration in the blood occur? again, use code to answer this question. 
...
md"🍕 the drug is ineffective if its concentration in the blood is less than 1.0 mg/L.
...
!!! hint
	use `vspan!` [here](https://docs.makie.org/dev/reference/plots/vspan) and apply this to the figure returned from `viz_response` via [here](https://simonensemble.github.io/Controlz.jl/dev/viz/#modifying-the-figures).
"
